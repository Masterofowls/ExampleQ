...
def login_student():
    ...
    if form.validate_on_submit():
        # Находим группу по выбранным параметрам
        group = Group.query.filter_by(
            city=form.city.data,
            course=int(form.course.data)
        ).filter(Group.name.like(f"%{form.group.data}%")).first()
...
def get_groups():
    ...
    if city and course:
        groups = Group.query.filter_by(city=city, course=int(course)).all()
        return jsonify([{'id': g.name, 'name': g.name} for g in groups])
